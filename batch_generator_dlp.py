...
LOCAL_CSV = "/tmp/transactions_batch.csv"
...
def mask_card_number(card_number):
    ...
    try:
        ...
        print(f"⚠️  DLP masking failed for {card_number}: {e}")
...
def generate_transaction():
    ...
    return {
        ...
        "masked_card_number": mask_card_number(card_number),  # 🔒 DLP masking
...
def upload_to_gcs(batch_id):
    blob_name = f"batches/batch_{batch_id}.csv"
...
def load_to_bigquery(gcs_uri):
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        schema=[
            ...
            bigquery.SchemaField("masked_card_number", "STRING"),  # 🔒 DLP masked field
...
def run_batch_pipeline(speed=10):
    print(f"🔒 DLP-Enabled Batch Pipeline — flushing every {BATCH_INTERVAL}s")
    print(f"Late arrival lookback: {LATE_LOOKBACK_MINUTES} extra minutes in fraud queries")
    print(f"Card numbers masked via Cloud DLP before storage")
    print("=" * 60)
